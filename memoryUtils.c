...
char retrieve6BitsValue(char* bytes, unsigned char index) {
    if (((index * 6) % 8) == 0) {
        ...
        return  ((((bytes[firstByteIndex] << ptrOnFirstByte) | //Retrieving value from the first byte and offsetting it
                (((bytes[firstByteIndex + 1] >> (8 - (ptrOnFirstByte))) & (unsigned char) pow(2, ptrOnFirstByte) - 1))) >> 2) & 0x3F); //Retrieving value from the second byte, offsetting it, remove the values from this byte that are not part of the value, offsetting it again and removing the last 2 bits
    }
}
...
char retrieve3BitsValue(char* bytes, unsigned char index) {
    if (((index * 3) % 8) == 0) {
        return ((bytes[(int) floor((index * 3) / 8)] >> 5) & 0b00000111);
    } else {
        unsigned char ptrOnFirstByte = (index * 3) % 8;
        ...
        return  ((((bytes[firstByteIndex] << ptrOnFirstByte) | //Retrieving value from the first byte and offsetting it
                (((bytes[firstByteIndex + 1] >> (8 - (ptrOnFirstByte))) & (unsigned char) pow(2, ptrOnFirstByte) - 1))) >> 5) & 0b00000111); //Retrieving value from the second byte, offsetting it, remove the values from this byte that are not part of the value, offsetting it again and removing the last 2 bits
    }
}
